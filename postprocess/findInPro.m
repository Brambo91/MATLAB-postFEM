function val = findInPro(name,i)
% this function finds the value of a variable in a pro file
% input: name, i
% value specified for i^th occurrance of 'name' is returned
% only the first *.pro file in the directory is used
% returns a double if possible, otherwise a string

try
  filename = ls('*.pro');
catch
  error(['no *.pro file found in directory ' pwd])
end
